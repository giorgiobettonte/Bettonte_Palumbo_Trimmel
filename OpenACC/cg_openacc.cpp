...
// OpenACC function to perform dot product
double dot(const double * x, const double * y, size_t size)
{
    double result = 0.0;

    #pragma acc parallel loop present(x[0:size], y[0:size]) reduction(+:result)
...
// OpenACC function to perform axpby
...
// OpenACC function to perform gemv
void gemv(double alpha, const double * A, const double * x, double beta, double * y, size_t num_rows, size_t num_cols)
{
    // y = alpha * A * x + beta * y;

    #pragma acc parallel loop present(A[0:num_rows*num_cols], x[0:num_cols], y[0:num_rows])
    for(size_t r = 0; r < num_rows; r++)
    {
        double y_val = 0.0;
...
void conjugate_gradients(const double * A, const double * b, double * x, size_t size, int max_iters, double rel_error)
{
    ...
    #pragma acc enter data copyin(A[0:size*size], b[0:size], x[0:size], r[0:size], p[0:size], Ap[0:size])

    // Initialize arrays with OpenACC
    #pragma acc parallel loop present(x[0:size], r[0:size], p[0:size], b[0:size])
    for(size_t i = 0; i < size; i++)
    {
        x[i] = 0.0;
        r[i] = b[i];
    ...
    // Free memory on the GPU and copy data back to host
    #pragma acc exit data copyout(A[0:size*size], b[0:size], x[0:size], r[0:size], p[0:size], Ap[0:size])
